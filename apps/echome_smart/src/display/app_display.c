...
static void __app_display_msg_handle(DISPLAY_MSG_T *msg_data)
{
    ...
    case TY_DISPLAY_TP_NETWORK: {
        ...
        // 同时更新navigation页面的WiFi状态
        extern void ui_navigation_set_network(char *wifi_icon);
        ui_navigation_set_network(wifi_icon);
    } break;
    case TY_DISPLAY_TP_CHAT_MODE: {
        ui_set_chat_mode(msg_data->data);
    } break;
#if defined(ENABLE_GUI_WAKEUP) && (ENABLE_GUI_WAKEUP == 1)
    case TY_DISPLAY_TP_WAKEUP_SHOW_WAIT: {
        ui_wakeup_show_wait();
    } break;
    case TY_DISPLAY_TP_WAKEUP_START_ANIMATION: {
        // 动画功能已移除，不再需要
        PR_INFO("Wakeup animation request ignored - animation removed");
    } break;
    case TY_DISPLAY_TP_WAKEUP_HIDE: {
        ui_wakeup_hide();
    } break;
    case TY_DISPLAY_TP_WAKEUP_TOUCH_EVENT: {
        // 触摸事件处理 - 可以在这里添加进入下一页面的逻辑
        PR_INFO("Wakeup touch event - transitioning to next page");
        ui_wakeup_hide();
        // TODO: 在这里添加进入聊天界面或其他页面的逻辑
    } break;
#endif
    default: {
...
static void __chat_bot_ui_task(void *args)
{
    ...
    // ui initialization
    TUYA_CALL_ERR_LOG(ui_manager_init(&sg_display.ui_font));
...
#endif
    tuya_lvgl_mutex_unlock();
    PR_DEBUG("ui init success - boot sequence started in ui_init");
...
// Wakeup UI convenience functions
#if defined(ENABLE_GUI_WAKEUP) && (ENABLE_GUI_WAKEUP == 1)
OPERATE_RET app_display_wakeup_show_wait(void)
{
    return app_display_send_msg(TY_DISPLAY_TP_WAKEUP_SHOW_WAIT, NULL, 0);
}

OPERATE_RET app_display_wakeup_start_animation(void)
{
    return app_display_send_msg(TY_DISPLAY_TP_WAKEUP_START_ANIMATION, NULL, 0);
}

OPERATE_RET app_display_wakeup_hide(void)
{
    return app_display_send_msg(TY_DISPLAY_TP_WAKEUP_HIDE, NULL, 0);
}

OPERATE_RET app_display_wakeup_handle_touch(void)
{
    return app_display_send_msg(TY_DISPLAY_TP_WAKEUP_TOUCH_EVENT, NULL, 0);
}
#endif /* ENABLE_GUI_WAKEUP */
