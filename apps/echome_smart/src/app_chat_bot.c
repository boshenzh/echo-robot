...
OPERATE_RET app_chat_bot_init(void)
{
    ...
    // 初始化MQTT管理器 - 暂时注释掉用于调试
    // extern int mqtt_manager_init(void);
    // mqtt_manager_init();
    ...
    // 如果使用wakeup UI，启动待机页面
#if defined(ENABLE_GUI_WAKEUP) && (ENABLE_GUI_WAKEUP == 1)
    // 延迟启动wakeup UI，确保其他初始化完成
    // 延迟启动wakeup UI，确保其他初始化完成
    // tal_sw_timer_start(1000, false, (TAL_SW_TIMER_CB)app_display_wakeup_show_wait, NULL);
    // 暂时注释掉，等wakeup UI完全集成后再启用
    PR_INFO("Wakeup UI will start in 1 second");
#endif
#endif
    return OPRT_OK;
}
