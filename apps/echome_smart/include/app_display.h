...
typedef enum {
    ...
    // wakeup UI
    TY_DISPLAY_TP_WAKEUP_SHOW_WAIT,
    TY_DISPLAY_TP_WAKEUP_START_ANIMATION,
    TY_DISPLAY_TP_WAKEUP_HIDE,
    TY_DISPLAY_TP_WAKEUP_TOUCH_EVENT,
...
/**
 * @brief Show wakeup wait page (static image, dimmed screen)
 * @return OPERATE_RET Result of the operation
 */
OPERATE_RET app_display_wakeup_show_wait(void);

/**
 * @brief Start wakeup animation sequence (wake1 -> wake2 loop)
 * @return OPERATE_RET Result of the operation
 */
...
/**
 * @brief Handle touch event during wakeup (for wait state transition)
 * @return OPERATE_RET Result of the operation
 */
